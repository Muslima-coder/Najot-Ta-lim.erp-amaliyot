...
const Navbar:FC<{collapse:boolean}> = ({collapse}) => {
  ...
  const items = [
    ...
    {key:"2", icon:<UngroupOutlined  className="!text-[22px]"/>, label:<Link className="text-[20px]" to={PATH.groups}>Guruxlar</Link>},
    {key:"3", icon:<UserOutlined  className="!text-[22px]"/>, label:<Link className="text-[20px]" to={PATH.teachers}>Ustozlar</Link>},
    {key:"4", icon:<OpenAIOutlined  className="!text-[22px]"/>, label:<Link className="text-[20px]" to={PATH.students}>O'quvchilar</Link>},
  ]
  useEffect(() => {
    instance().get("/user/me").then(res => setMe(res.data))
  },[])
  return (
    <div className={`${collapse ? "w-[80px]": "w-[18%]"} overflow-y-auto duration-200 h-[100vh] bg-[#001529]`}>
      <div className="p-3 border-b-[1px] mb-[10px] flex items-center text-[#bc8e5b] gap-[15px] border-white">
        <LogoIcon classList="!w-[70px]" />
        {collapse ? "" : <span className="text-white text-[23px] font-medium capitalize">{me.role?.toLowerCase()}</span>}
      </div>
      <Menu
